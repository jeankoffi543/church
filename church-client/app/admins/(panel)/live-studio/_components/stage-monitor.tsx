      ...
      <div ref={stageRef} className="absolute inset-0">
        ...
        <AnimatePresence>
          {visible.map((layer, idx) => {
            ...
            // Media keep a STABLE key so an animation replay (animNonce bump) doesn't
            // remount them — a camera remount re-runs getUserMedia and flashes black.
            const isMedia =
              layer.type === "camera" || layer.type === "video" || layer.type === "image";

            return (
              <CompositeLayer
                key={isMedia ? layer.id : `${layer.id}-${animNonce}`}
                layer={effective}
