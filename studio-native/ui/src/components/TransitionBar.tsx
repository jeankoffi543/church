...
/** One symmetric navigation row: `[<] label [>]` — ported 1:1 from the web. */
function NavRow({
  label,
  PrevIcon,
  NextIcon,
  ...
  return (
    <div className="flex items-center gap-1">
      <button type="button" onClick={onPrev} disabled={disabled || !canPrev} title={`${label} précédent`} className={arrow}>
        <PrevIcon className="size-3.5 shrink-0 text-gold" strokeWidth={2.2} />
      </button>
      <span className="flex-1 text-center text-[11px] font-bold whitespace-nowrap text-white">{label}</span>
      <button type="button" onClick={onNext} disabled={disabled || !canNext} title={`${label} suivant`} className={arrow}>
...
/**
 * Center transition column between the Preview and Program monitors — ported 1:1
 * from the web console's `transition-bar.tsx`: CUT (send scene on air), Écran
 * vide (black-out toggle), and the bible verse/chapter nav (shown when a visible
 * bible source exists — wired later).
 */
